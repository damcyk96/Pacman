...
class Ghost extends Component {
  ...
  move = () => {
    ...
    if (this.state.direction === "up") {
      this.setState({
        position: { top: Math.max(currentTop - this.props.step, 0), left: currentLeft },
      });
      ...
      this.setState({
        position: {
          top: Math.min(
            currentTop + this.props.step,
            window.innerHeight - this.props.size - this.props.border - this.props.topScoreBoard
          ),
          left: currentLeft,
        },
      });
      ...
      this.setState({
        position: { top: currentTop, left: Math.max(currentLeft - this.props.step, 0) },
      });
    }
